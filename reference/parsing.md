...
- **Solo founder/operator with sustained, quantified, multi-year achievement - P4 vs P5 (NEW 2026-08-23)**: the Founder decision tree's "solo operator -> P-track" default lands on P4 by itself, but a solo operator whose bullets show a *sustained, company-defining technical achievement over several years* (repeated competition wins, a multi-year compounding performance metric, a durable product/strategy that outlived the stint) reads closer to P5's "develops new approaches to solve highly complex, unique, or ambiguous problems" and "fully self-directed" dimensions than to P4's single-domain-ownership scope - even with zero reports. Do not default to P4 by reflex; weigh the duration and magnitude of the achievement. Always flag this as a borderline P4/P5 call in `data_quality` regardless of which way it lands, since "solo + years of sustained results" is inherently a judgment call, not a clean rule match. Confirmed via a real-world test parse (a Founder/Director of a solo algorithmic-trading firm, two-time national trading-competition winner, ~70% CAGR sustained over six years of live trading - landed P5 over the P-track default).
...
### Boomerang re-engagement at a former, non-adjacent employer (NEW 2026-08-23)
A candidate sometimes returns to an employer years after an earlier stint there, with a full separate career chapter (different employer, different role) in between - e.g. Senior Data Scientist at Company X in 2016-2017, several other employers in between, then an ML Architect contract back at Company X starting in 2025. This is NOT the same-employer staircase pattern (`connector.same_employer_as_previous`), which is for chronologically *adjacent* roles at one employer (promotions, lateral moves) - a boomerang return is chronologically adjacent to whatever employer came right before it, not to the candidate's own earlier stint at the same company.

Handling:
- Set `connector.previous_role_id` to the chronologically adjacent role (whatever the candidate did immediately before this one), exactly as normal - do NOT point it at the earlier stint at the same employer.
- Add a `data_quality` entry cross-referencing both role IDs, noting the gap and that the two stints are non-adjacent (e.g. "role_2 is a boomerang return to the employer of role_6, ~8 years and several employers apart - not a continuation of that earlier tenure").
- The visualization phase still draws a same-employer staircase between the two stints (it groups by `company` across the *entire* role list, not by adjacency - see `reference/visualization.md`'s Same-employer staircase section) - that is expected and correct; the staircase communicates "same employer," not "continuous tenure." Consider adding a short italic clarifying note under the company name on the more recent stint (e.g. "Second engagement - previously Senior Data Scientist, Mar 2016 - Nov 2017") so a reader doesn't misread the staircase as one unbroken run.

Confirmed via a real-world test parse (a CTO candidate with a 2016-2017 Senior Data Scientist stint at an employer, followed by ~8 years and 5 other employers, then a 2025-present contract ML Architect engagement back at the same employer).

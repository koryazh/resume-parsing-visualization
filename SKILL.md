...
- `docs/visualization-technical-spec.md` - the portable technical spec for the visualization phase; also useful outside this skill system. Current version 1.7 (2026-09-04): adds the full-career synthesis block (a composed `candidate.career_synthesis` paragraph rendered centered above the ladder, collapsed by default behind a disclosure toggle), a timed attribution banner documented as a customizable default rather than a locked rule, and a `Save as PDF` control backed by a print stylesheet that strips the interactive chrome and both tiers of AI synthesis panel - on top of v1.6's C-Level peak-label collapse and boomerang re-engagement notes, v1.5's `C-Level` → `C` axis-overlay shortening, v1.2's default solid dominant-family-color bar encoding (striped remains a documented alternate), and locked page-density/divider/hero-alignment defaults.
